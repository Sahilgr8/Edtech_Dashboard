            ...
            <div v-if="this.sub=='ALL'">
                ...
                <q-card-section class=" tw-h-96">
                    <q-toolbar class=" tw--mt-2">
                        ...
                        <q-select @input="changeChartData" dense outlined v-model="sub" class=" tw-w-40" :options="subjects" />
                    </q-toolbar>
                    
                    <IEcharts class=" tw--mt-2" style="height:320px" :option="alltestBar" :resizable="true"/></q-card-section></q-card></div>
          <div v-else><q-card ><q-card-section class=" tw-h-96"><q-toolbar class=" tw--mt-2">
                        <q-toolbar-title class=" tw-font-bold tw-text-gray-700">
                           Test Details
                        </q-toolbar-title>
                        <q-select @input="changeChartData" dense outlined v-model="sub" class=" tw-w-40" :options="subjects" />
                    </q-toolbar><IEcharts class=" tw--mt-2" :option="testBar" style="height:320px" :resizable="true"/></q-card-section></q-card></div>
            <div class=" xs:tw-col-span-1 md:tw-col-span-2"><q-table title="TOTAL TESTS" :data="test_data" :columns="col" hide-pagination/></div>
        </div>
    </q-page>
</template>
...
export default {
    ...
    methods:{
        changeData(){
            ...
            // else if(this.div!='ALL' && this.subject!='ALL'){
            //     if(this.subject=='Maths'){
            //         this.top_students_data=this.all_data.sort((a,b)=>b.maths-a.maths).filter(e=>e.div==this.div && e.class==this.clas).slice(0,10)
            //     }else if(this.subject=='Science'){
            //         this.top_students_data=this.all_data.sort((a,b)=>b.science-a.science).filter(e=>e.div==this.div && e.class==this.clas).slice(0,10)
            //     }else if(this.subject=='English'){
            //         this.top_students_data=this.all_data.sort((a,b)=>b.english-a.english).filter(e=>e.div==this.div && e.class==this.clas).slice(0,10)
            //     }else if(this.subject=='Hindi'){
            //         this.top_students_data=this.all_data.sort((a,b)=>b.hindi-a.hindi).filter(e=>e.div==this.div && e.class==this.clas).slice(0,10)
            //     }else if(this.subject=='Social Studies'){
            //         this.top_students_data=this.all_data.sort((a,b)=>b.social-a.social).filter(e=>e.div==this.div && e.class==this.clas).slice(0,10)
            //     }
            // }else if(this.div=='ALL' && this.subject=='ALL'){
            //    this.top_students_data=this.all_data.sort((a,b)=>b.total-a.total).filter(e=>e.class==this.clas).slice(0,10)
            // }
        },
        changeChartData(){
